...
void sumMatrix(double firstMatrix[][MATRIX_SIZE], double secondMatrix[][MATRIX_SIZE], double sumMatrix[][MATRIX_SIZE]){
    for(int i = 0; i < MATRIX_SIZE; i++){
        for(int j = 0; j < MATRIX_SIZE; j++){
            ...
            if(sumMatrix[i][j] < 0.000000001 && sumMatrix[i][j] > -0.000000001){
                sumMatrix[i][j] = 0;
            }
        }
    }
    ...
    for(int i = 0; i < MATRIX_SIZE; i++){
        for(int j = 0; j < MATRIX_SIZE; j++){
            for(int k = 0; k < MATRIX_SIZE; k++){
                ...
                if(multiMatrix[i][j] < 0.000000001 && multiMatrix[i][j] > -0.000000001){
                    multiMatrix[i][j] = 0;
                }
            }
        }
...
void inverseMatrix(double matrix[MATRIX_SIZE][MATRIX_SIZE], double inverseMatrix[MATRIX_SIZE][MATRIX_SIZE]){
    ...
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
            inverseMatrix[j][i] = ((matrix[(j + 1) % 3][(i + 1) % 3] * matrix[(j + 2) % 3][(i + 2) % 3])
                                   - (matrix[(j + 1) % 3][(i + 2) % 3] * matrix[(j + 2) % 3][(i + 1) % 3]))/ determinant;
           
            if(inverseMatrix[j][i] < 0.000000001 && inverseMatrix[j][i] > -0.000000001){
                inverseMatrix[j][i] = 0;
            }
        }
    }
...
int main(int argc, const char * argv[]) {
    double A[MATRIX_SIZE][MATRIX_SIZE] = { -9,  2, 1,
                                           1,  -9,  -1,
                                           -1, 0,  -9};
    
    double C[MATRIX_SIZE][MATRIX_SIZE] = {0, 58, 0,
                                          58, 58, 0,
                                          58, 0, 58};
